 ...
 * With an `image`, it's an asymmetric split — the photo (or, with `video`
 * set, an autoplaying video using `image` as its poster) full-bleeds on its
 * own side, the caption sits in open space on solid ink-950, never printed
 * on top of the media. Without an `image`, it falls back to the flat
...
export function DarkHero({
  ...
  image?: string;
  imageAlt?: string;
  /** When set (and `image` is present), an autoplaying background video replaces the static image; `image` becomes its poster frame. */
  video?: string;
